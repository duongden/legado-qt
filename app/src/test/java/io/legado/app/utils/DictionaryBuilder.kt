...
class DictionaryBuilderRunner {
    @org.junit.Test
    fun compileDict() {
        val inputDir = java.nio.file.Paths.get("src/main/assets/translate/vietphrase")
        val outputDir = inputDir
        DictionaryBuilder().processDirectory(inputDir, outputDir)
        compilePhonetic(inputDir)
        println("Dictionary compilation from runner completed!")
    }

    private fun compilePhonetic(dir: java.nio.file.Path) {
        val txtFile = java.io.File(dir.toFile(), "ChinesePhienAmWords.txt")
        val binFile = java.io.File(dir.toFile(), "ChinesePhienAmWords.bin")
        if (!txtFile.exists()) return

        val PHONETIC_MAGIC = 0x50484D50
        val PHONETIC_VERSION = 1
        val map = java.util.HashMap<String, String>()

        java.io.BufferedReader(java.io.InputStreamReader(java.io.FileInputStream(txtFile))).use { reader ->
        ...
        java.io.DataOutputStream(java.io.BufferedOutputStream(java.io.FileOutputStream(binFile))).use { dos ->
        ...
        println("Phonetic binary created: ${binFile.length()} bytes")
    }
}
