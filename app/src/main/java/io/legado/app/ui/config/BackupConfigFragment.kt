...
class BackupConfigFragment : PreferenceFragment(),
    ...
    private val restoreDoc = registerForActivityResult(HandleFileContract()) {
        it.uri?.let { uri ->
            waitDialog.setText(getString(R.string.restoring))
            waitDialog.show()
    ...
    private fun backup(backupPath: String) {
        waitDialog.setText(getString(R.string.backing_up))
        waitDialog.setOnCancelListener {
            backupJob?.cancel()
        }
        waitDialog.show()
        ...
        backupJob = lifecycleScope.launch {
            try {
                ...
                ensureActive()
                AppLog.put(getString(R.string.backup_error, e.localizedMessage), e)
                appCtx.toastOnUi(
                    appCtx.getString(
    ...
    fun restore() {
        ...
        Coroutine.async {
            ...
            AppLog.put(getString(R.string.restore_webdav_error, it.localizedMessage), it)
            if (context == null) {
                return@onError
            }
            alert {
                setTitle(R.string.restore)
                setMessage(getString(R.string.webdav_error_local_restore, it.localizedMessage))
                okButton {
                    restoreFromLocal()
                }
                cancelButton()
    ...
    private suspend fun showRestoreDialog(context: Context) {
        ...
        if (AppWebDav.isJianGuoYun && names.size > 700) {
            context.toastOnUi(R.string.jianguoyun_limit_warning)
        }
        if (names.isNotEmpty()) {
    ...
    private fun restoreWebDav(name: String) {
        ...
        val task = Coroutine.async {
            ...
            AppLog.put(getString(R.string.webdav_restore_error, it.localizedMessage), it)
            appCtx.toastOnUi(getString(R.string.webdav_restore_error, it.localizedMessage))
        }.onFinally {
            waitDialog.dismiss()
        }
        waitDialog.setOnCancelListener {
