...
object WebBook {
    ...
    suspend fun runPreUpdateJs(bookSource: BookSource, book: Book, isFromBookInfo : Boolean = false): Result<Unit> {
        return kotlin.runCatching {
            ...
            AppLog.put("Thực thi quy tắc preUpdateJs thất bại Nguồn sách:${bookSource.bookSourceName}", it)
        }
    }
    ...
    suspend fun getContentAwait(
        ...
        if (contentRule.content.isNullOrEmpty()) {
            Debug.log(bookSource.bookSourceUrl, "⇒Quy tắc nội dung trống, sử dụng liên kết chương:${bookChapter.url}")
            return bookChapter.url
        }
        if (bookChapter.isVolume && bookChapter.url.startsWith(bookChapter.title)) {
            Debug.log(bookSource.bookSourceUrl, "⇒Nội dung danh mục cấp một không phân tích quy tắc")
    ...
    suspend fun preciseSearchAwait(
        ...
        return kotlin.runCatching {
            ...
            throw NoStackTraceException("Không tìm thấy sách $name($author)")
        }.onFailure {
            currentCoroutineContext().ensureActive()
        }
    }
    ...
    private fun checkRedirect(bookSource: BookSource, response: StrResponse) {
        response.raw.priorResponse?.let {
            if (it.isRedirect) {
                Debug.log(bookSource.bookSourceUrl, "≡Phát hiện chuyển hướng(${it.code})")
                Debug.log(bookSource.bookSourceUrl, "┌Địa chỉ sau khi chuyển hướng")
